...
<style>
    .shop {
    background-image: url('assets/images/bg.jpeg'); /* Path to your image */
...
<script>
    document.getElementById('in_small_group').addEventListener('change', function () {
        ...
        if (this.value === 'yes') {
            ...
            groupNumberSelect.removeAttribute('required'); 
    ...
    for (var j = 0; j < selectFields.length; j++) {
        // Ensure validation skips hidden fields (like group_number when not needed)
        if (selectFields[j].hasAttribute("required") && selectFields[j].value == "" && selectFields[j].offsetParent !== null) {
